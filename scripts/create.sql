...
CREATE TABLE produto (
    ...
    preco DECIMAL(10,2),
    nome VARCHAR(100),
    FOREIGN KEY (codigo_categoria) REFERENCES categoria(codigo) ON DELETE SET NULL
);
...
CREATE TABLE fornecedor (
    ...
    nome VARCHAR(100),
    FOREIGN KEY (id_fornecedor) REFERENCES pessoa(id_pessoa) ON DELETE CASCADE
);
...
CREATE TABLE funcionario (
    ...
    funcao VARCHAR(50),
    id_supervisor INT,
    FOREIGN KEY (id_funcionario) REFERENCES pessoa(id_pessoa) ON DELETE CASCADE,
    FOREIGN KEY (id_supervisor) REFERENCES funcionario(id_funcionario) ON DELETE SET NULL
);
...
CREATE TABLE historico_preco_produto (
    ...
    preco DECIMAL(10,2),
    data DATETIME,
    FOREIGN KEY (codigo_produto) REFERENCES produto(id_produto) ON DELETE CASCADE
);
...
CREATE TABLE telefone (
    ...
    id_telefone INT,
    PRIMARY KEY (numero),
    FOREIGN KEY (id_telefone) REFERENCES pessoa(id_pessoa) ON DELETE CASCADE
);
...
CREATE TABLE compra (
    ...
    FOREIGN KEY (fk_produto_id) REFERENCES produto(id_produto) ON DELETE CASCADE,
    FOREIGN KEY (fk_cliente_id) REFERENCES cliente(id_cliente) ON DELETE CASCADE,
    FOREIGN KEY (fk_operacional_id_funcionario) REFERENCES funcionario(id_funcionario) ON DELETE CASCADE,
...
CREATE TABLE entrega (
    ...
    data_reposicao DATE,
    FOREIGN KEY (fk_fornecedor_id) REFERENCES fornecedor(id_fornecedor) ON DELETE CASCADE,
    FOREIGN KEY (fk_produto_id) REFERENCES produto(id_produto) ON DELETE CASCADE,
